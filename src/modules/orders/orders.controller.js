...
export const getOrders = async (req, res) => {
  try {
    ...
    if (!orders)
      return res.status(404).json({ success: false, message: 'No orders found yet', data: {} });
...
export const addOrder = async (req, res) => {
  const { customer, products, payment_method, city, country, postal_code, phone_number } = req.body;
  const isAvailable = await checkAvailability(products);
  if (!isAvailable)
    return res
      .status(400)
      .json({ success: false, message: 'Not enough stock for the requested quantity', data: {} });
  ...
  try {
    ...
    return res
      .status(201)
      .json({ success: true, message: 'Order was successfully added', order: orderPlaced });
  } catch (error) {
    return serverErrorResponse(res, error);
  } finally {
    await mongoose.disconnect();
